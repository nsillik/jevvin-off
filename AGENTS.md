...
- Pitfall: The ANE bundles are fixed-shape and **refuse** a request over 96 tokens rather than truncating it, so a long or non-English post is dropped as `capacity`. The client-side wording sets the rate: described options and five sentiment levels measured 36%, the shipped terse questions 16% (too-long posts and undeclared-language ones — the `langs=en` filter cannot catch those). `sidecar.py` counts tokens through the library's own `prepare` path before calling the model, so a refusal costs no inference. `--model=aac6fef/laya-multilingual-coreml` (1024 tokens, CPU + GPU) refuses nothing; see the shape-switching note below.
- Pitfall: The 1024-token bundle's export is enumerated-shape (`flexible: true`, buckets 16…1024), and Core ML pays ~285ms whenever a request lands in a different bucket from the previous one but ~17ms when it repeats — measured. A live stream alternates lengths, so that bundle measured 345ms/post end to end while a pinned shape costs 31ms at the full 1024 tokens. Pinning (fixed-shape re-export, or padding every request into one bucket) is what makes the big-context bundle usable.
...
- Pitfall: Laya is a 322M ModernBERT-style encoder with 2-layer decision heads, not a decoder — one pass, three heads, no reasoning and no world knowledge beyond its pretraining. Its judgments are **not** Jev's judgments on the same questions. Measured on one short ambiguous post: Laya said spam 0.94 / tech / most-negative tone at 0.86-0.94 confidence; hosted Jev said spam 0.02 / sports 0.70 / mixed-neutral 0.84. Rewording the questions moved `topic` only to `other`, and dropping the emoji *raised* spam confidence to 0.98. Confident-and-wrong is the failure mode to expect, so do not act on these answers and do not read the example's output as evidence of accuracy. The port's own validation is conversion fidelity (59/59 fixtures), explicitly not task accuracy.
